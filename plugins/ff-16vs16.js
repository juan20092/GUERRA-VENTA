...
let handler16vs16 = async (m, { conn, args }) => {
  if (!args.length) return conn.sendMessage(m.chat, { text: '❌ Debes indicar hora y país. Ej: .16vs16 3 pm mx' })

  let lastArg = args[args.length - 1].toLowerCase().replace(/,$/, '')
  let zonaInput = aliasesMX16.includes(lastArg) ? 'mx' : aliasesCO16.includes(lastArg) ? 'co' : null
  ...
  if (!match) return conn.sendMessage(m.chat, { text: '❌ Hora inválida. Ej: .16vs16 3 pm mx' })
  ...
  versusData16[sent.key.id] = { chat: m.chat, escuadra: [], suplentes: [], mexText, colText }
}
...
conn.ev.on('messages.upsert', async ({ messages }) => {
  for(let msg of messages){
    ...
    if(emoji==='❤️'){ if(data.escuadra.length<16) data.escuadra.push(user) }
    else if(emoji==='👍'){ if(data.suplentes.length<8) data.suplentes.push(user) }
    else continue
